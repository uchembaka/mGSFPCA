...
#' the estimated principal components, basis functions, and other model parameters.
#' @param eval_pts Numeric vector specifying the time points at which to evaluate the
#' principal component functions.
#'
#' @return A matrix of dimension \code{length(eval_pts) x p}, where \code{p} is the number
#' of principal components, containing the evaluated principal component functions at
#' the specified \code{eval_pts}.
#'
#' @details
...
eval_mGSFPCA <- function(mGSFPCA_obj, eval_pts = NULL) {
  ...
  } else if (all(eval_pts == mGSFPCA_obj$pars$workGrid)) {
