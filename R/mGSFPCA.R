...
#'
#' @param data A matrix or data frame with three columns: ID (subject identifier), time (observation
#' time points), and value (observed values).
#' @param p Integer vector specifying the candidate number of principal components to consider.
#' Default is 2:5.
#' @param k Integer vector specifying the candidate number of basis functions to consider.
#' Default is c(5, 10, 15).
#' @param basis_type Character string specifying the type of basis functions to use. Options are
#' "bspline" (default) or "fourier".
#' @param maxit Integer specifying the maximum number of iterations for the optimization
#' algorithm. Default is 500.
#' @param optim_tol Numeric specifying the tolerance for the optimization convergence.
#' Default is 1e-5.
#' @param optim_trace Integer specifying the level of tracing information from the optimization
#' algorithm (Based on optim()). Default is 0 (no tracing).
#' @param nRegGrid Integer specifying the number of points in the regular grid for evaluation.
#' Default is 51.
#' @param init_coeff Numeric vector of initial coefficients for optimization. If NULL (default),
#' coefficients are initialized randomly.
#' @param obs_range Numeric vector of length 2 specifying the observation range (aT, bT). If
#' NULL (default), it is set to c(0,1).
#' @param mu_nbasis Integer specifying the number of basis functions for the mean function.
#' Default is 15.
#' @param bin_size Integer specifying the number of bins for data binning. Default is 51.
#' @param use_kp_grid Logical indicating whether to evaluate all combinations of p and k
#' (TRUE, default) or use a stepwise optimization approach (FALSE).
#' @param skip_check Logical indicating whether to skip input validation checks. Default is
#' FALSE.
#'
...
#' \item \code{convergence}: Convergence status of the optimization (Based on optim()).
#' }
#' }
#'
#' @details
#' The mGSFPCA function implements a Modified Gram-Schmidt approach to functional
#' principal component analysis (FPCA) using MLE.
#'
#' @examples
...
#' fit$Lambda
#'
#' plot(grid, fit$Phi[,1], type ='l')
#' lines(grid, -bspline_sim$eigFun[1,], col = 2) # change sign if needed
#' min(Metrics::rmse(fit$Phi[,1], bspline_sim$eigFun[1,]), Metrics::rmse(-fit$Phi[,1], bspline_sim$eigFun[1,]))
#'
#' x_pred <- get_xHat(fit)
#' Metrics::rmse(x_pred$xHat, bspline_sim$X)

#'
#' result <- mGSFPCA(bspline_sim$Y, p = 2:5, k = c(5, 10, 15), basis_type = "bspline")
#' str(result)
#'
#' @references
#' Peng, J. and Paul, D. (2009). A geometric approach to maximum likelihood estimation
#' of the functional principal components from sparse longitudinal data.
...
mGSFPCA <- function(data,
  ...
  list(
    ...
    pars = list(
      ...
      coeffs = cvec,
      mu_fdobj = comp_data$mu_fdobj,
      mu_basis = comp_data$mu_basis,
      workGrid = comp_data$est_pts,
      evalGrid = inputs$evalGrid,
      eigBasis = basis,
      binData = comp_data$newData,
