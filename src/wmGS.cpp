...
inline double weighted_inner_product(const VectorXd& x, const VectorXd& y, const VectorXd& weights) {
  return (x.array() * weights.array() * y.array()).sum();
}


// Weighted norm
inline double weighted_norm(const VectorXd& x, const VectorXd& weights) {
  return std::sqrt(weighted_inner_product(x, x, weights));
}


// [[Rcpp::export]]
Eigen::MatrixXd mwGS(const MatrixXd& U, const VectorXd& weights) {
  const int D = U.rows();
  const int d = U.cols();
